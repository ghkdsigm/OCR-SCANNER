...
export class VehicleRegistrationTextParser {
  ...
  private static extractCarType(text: string): string {
    ...
    if (labeled) return this.normalizeCarTypeValue(labeled[1])
  ...
  private static extractCarName(text: string, lines: string[]): string {
    for (let i = 0; i < lines.length; i++) {
      ...
      if (this.isLikelyCarName(sameLine)) return this.cleanCarName(sameLine)

      for (const candidate of [lines[i + 1] ?? '', lines[i + 2] ?? '']) {
        if (this.isLikelyCarName(candidate)) return this.cleanCarName(candidate)
      }
    }
